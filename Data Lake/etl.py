...
from pyspark.sql.types import StringType, DoubleType, StructType, StructField, LongType, TimestampType, DateType
...
def process_song_data(spark, input_data, output_data):
    ...
    artists_table = df.selectExpr("artist_id", "artist_name as name", "artist_location as location", 
                            "artist_latitude as latitude", "artist_longitude as longitude").drop_duplicates(subset=['artist_id'])
...
def process_log_data(spark, input_data, output_data):
    ...
    time_table = df.select(F.col("timestamp").alias("start_time"), F.year("timestamp").alias("year"), F.hour("timestamp").alias("hour")
                      , F.dayofmonth("timestamp").alias("day"),
                       F.month("timestamp").alias("month"),
                       F.weekofyear("timestamp").alias("weekofyear"), 
                       F.dayofweek("timestamp").alias("weekday")).drop_duplicates(subset=['start_time'])
    ...
    year_month = udf(lambda x: "%s-%s" % (x.year, x.month))

    # extract columns from joined song and log datasets to create songplays table 
    songplays_table = df.join(song_df, df.song == song_df.song_id, "left") \
        .select("timestamp",F.year("timestamp").alias("year"),F.month("timestamp").alias("month"),
